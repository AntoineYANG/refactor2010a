...
def get_best(pop,fitness):
    ...
    return pop[index]
def PSO(car_count,car_load,r,q,lis,pop_size,iteration,w,c1,c2):#数据#粒子数#迭代次数 （粒子数不能超过50）
    file_handle = open ( '优化过程.txt' , mode = 'w' )
    line = "参数设置："+" pop_size = "+str(pop_size)+" iteration = "+str(iteration)+" w = "+str(w)+" c1 = "+str(c1)+" c2 = "+str(c2)+" car_count = "+str(car_count)+" car_load = "+str(car_load)+" r = "+str(r)+"\n" 
    file_handle.write( line )
    #print("参数设置："+" pop_size = "+str(pop_size)+" iteration = "+str(iteration)+" w = "+str(w)+" c1 = "+str(c1)+" c2 = "+str(c2)+" car_count = "+str(car_count)+" car_load = "+str(car_load)+" r = "+str(r))
    gbestbit = []
    gbestfit = []
    pop =[] #种群
    pbest = [] #局部最优
    ...
    while (t<iteration):
        ...
        line1 = "路径"+str(gbest[t])+"第"+str(t)+"代的全局最优为:"+str(fit(gbest[t],lis,dis,car_count,car_load,r))+"\n"
        file_handle.write( line1 )
        
        #print("路径"+str(gbest[t])+"第"+str(t)+"代的全局最优为:"+str(fit(gbest[t],lis,dis,car_count,car_load,r)))
        qq = []
        gbestbit.append(t)
